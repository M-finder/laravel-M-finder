...
layui.define(['layer', 'laytpl', 'form', 'upload', 'util'], function (exports) {
    ...
    var gather = {
        ...
        , layEditor: function (options) {
            ...
            music = '';
            if(query_dir.indexOf('/userhome') >= 0 || query_dir.indexOf('/admin') >= 0){
                music = '<span type="music" title="插入音乐"><i class="layui-icon">&#59132;</i>音乐</span>';
            }
                ...
                , music: function (editor) { //插入代码
                            layer.open({
                                ...
                                , content: ['<ul class="layui-form layui-form-pane" style="margin: 20px;">'
                                            ...
                                            , '<div class="layui-input-block layui-music-input">'
                                            , '<input required name="name" placeholder="音乐名称" value="" class="layui-input">'
                                            , '</div>'
                                            , '</li>'
                                            , '<li class="layui-form-item">'
                                            , '<label class="layui-form-label">音乐地址</label>'
                                            , '<div class="layui-input-block layui-music-input">'
                                            , '<input required name="music" placeholder="远程音乐地址" value="" class="layui-input">'
                                            , '</div>'
                                            , '</li>'
                                            , '<li class="layui-form-item">'
                                            , '<label class="layui-form-label">封面地址</label>'
                                            , '<div class="layui-input-block">'
                                            , '<input required name="image" placeholder="音乐封面地址" value="" class="layui-input">'
                                            , '</div>'
                                            , '</li>'
                                            , '<li class="layui-form-item" style="text-align: center;">'
                                            , '<button type="button" lay-submit lay-filter="Music" class="layui-btn">确认</button>'
                                            , '</li>'
                                            , '</ul>'].join('')
                                , success: function (layero, index) { 
                                    form.on('submit(Music)', function (data) {
                                        var name = layero.find('input[name="name"]');
                                        var music = layero.find('input[name="music"]');
                                        var image = layero.find('input[name="image"]');
                                        var field = data.field;
                                        
                                        if (!field.name){return name.focus();}
                                        if (!field.music){return music.focus();}
                                        if (!field.image){return image.focus();}
                                            
                                        layui.focusInsert(editor[0],'music[name="'+field.name+'",src="'+field.music+'",image="'+field.image+'"]');
                                        layer.close(index);
                                    });
                                }
                            });
        ...
        , content: function (content) {
            ...
            content = gather.escape(content || '') //XSS
                    .replace(/music\[(.*?)\]/g, function (music){
                        res = music.match(/&quot;(.*?)&quot;/g);
                        return  !res ? '' : '<div class="player"><div class="musicplayerslogo"><div class="musicplayersavatar"><a><img alt="" src='+res[2].replace(/\&quot;/g, "")+'></a></div><div class="statusinfo"><span id="playericon-play" class="playericon-play"></span>'+res[0].replace(/\&quot;/g, "")+'</div></div><audio id="houlai" src='+res[1].replace(/\&quot;/g, "")+' class="playerd"></audio></div>';
                    })
